...
const verifyAccess = require("../middleware/permission");
...
// @access Public
router.post("/add", verifyToken, verifyAccess, async (req, res) => {
  ...
  try {
    ...
    if (product)
      return res
        .status(400)
        .json({ success: false, message: "Can not add an existed product!" });
    ...
    res.json({
      success: true,
      message: "Add a product successfully",
      product: product,
    });
  } catch (error) {
    console.log(error);
...
// @desc Update product
// @access Public
router.put("/update/:id", verifyToken, verifyAccess, async (req, res) => {
  const { name, description, price, url, quantity, category } = req.body;
...
// @desc delete product
// @access Public
router.delete("/delete/:id", verifyAccess, verifyAccess, async (req, res) => {
  try {
    ...
    if (!deletedProduct) {
      return res
        ...
        .json({
          success: false,
          message: "Cannot delete this product or this product is not existed!",
        });
    }
